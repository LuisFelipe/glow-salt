...
from layers import ActNorm, CouplingLayer, FactorOut, PositionalAdd
...
class SegFlowBert(tf.keras.Model):
    """
    ...
    def __init__(self):
        ...
        """
        super(SegFlowBert, self).__init__()
        self.built = False
        self.config = Config()
        self.init_configs()
        self.optimizer = None
    ...
    def init_layers(self):
        ...
        for l in range(self.levels):
            positional_add = [PositionalAdd(name="l.{}-PositionalAdd".format(l))]
            self._blocks.append(positional_add)
    ...
    def loss_and_metrics(self, predicted, target):
        ...
        y_pred = tf.cast(tf.greater_equal(y_logits, 0.5), dtype=tf.float32)
        acc = tf.cast(tf.equal(y_pred[:, :, :, 0], y_mask[:, :, :, 0]), tf.float32)
        acc = tf.reduce_mean(
            acc, 
            axis=[1, 2]
        )
